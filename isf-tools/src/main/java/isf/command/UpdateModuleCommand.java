...
public class UpdateModuleCommand extends AbstractCommand {
	...
	protected void init() {
		...
		// main.project.
		if (main.projectSet && this.directorySet == false)
		{
			this.directory = new File(main.getProject(), "module");
		}
	...
	public UpdateModuleCommand(Main main) {
		super(main);
		configure();
	}
	...
	private void updateModule(String moduleName, File moduleDirectory) {
		...
		if (new File(moduleDirectory, moduleName + "-module-configuration.owl").exists())
		{
			try
			{
				configurationOntology = ISFTUtil.loadOntology(new File(moduleDirectory, moduleName
		...
		} else if (new File(moduleDirectory, moduleName + "-module-annotation.owl").exists())
		{
			try
			{
				annotationOntology = ISFTUtil.loadOntology(new File(moduleDirectory, moduleName
		...
		{
			int index = annotationOntology.getOntologyID().getOntologyIRI().toString()
					.lastIndexOf('/');
		...
		// build iris
		IRI topIri = IRI.create(iriPrefix + moduleName + ISFT.TOP_IRI_SUFFIX);
		IRI configurationIri = IRI.create(iriPrefix + moduleName + ISFT.CONFIGURATION_IRI_SUFFIX);
		IRI includeIri = IRI.create(iriPrefix + moduleName + ISFT.MODULE_INCLUDE_IRI_SUFFIX);
		IRI excludeIri = IRI.create(iriPrefix + moduleName + ISFT.MODULE_EXCLUDE_IRI_SUFFIX);
		IRI legacyIri = IRI.create(iriPrefix + moduleName + ISFT.MODULE_LEGACY_IRI_SUFFIX);
		...
		if (annotationConfiguration)
		{
			configurationOntology = ISFTUtil.createOntology(configurationIri, man);
		...
		} else if (axioms.size() == 0)
		{
			man.applyChange(new AddOntologyAnnotation(configurationOntology, df.getOWLAnnotation(
					...
					df.getOWLLiteral(iriPrefix + moduleName + ISFT.MODULE_IRI_SUFFIX))));
		}
		...
		} else if (axioms.size() == 0)
		{
			man.applyChange(new AddOntologyAnnotation(configurationOntology, df.getOWLAnnotation(
					...
					df.getOWLLiteral(iriPrefix + moduleName + ISFT.MODULE_IRI_INRERRED_SUFFIX))));
		}
		...
		{
			man.applyChange(new AddOntologyAnnotation(configurationOntology, df.getOWLAnnotation(
					ModuleVocab.module_file_name.getAP(),

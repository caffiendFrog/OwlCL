...
public enum ISFT {
	...
	prefix(ISFT_PREFIX), exclude(ISFT_EXCLUDE), exclude_subs(ISFT_EXCLUDE_SUBS), include(
			...
			ISFT_INCLUDE_INSTANCES), iri_mapps_to(ISFT_IRI_MAPPES_TO), builder_inferred(
			BUILDER_INFERRED), builder_uninferred(BUILDER_UNINFERRED), module_file_name(
			MODULE_FILE_NAME), module_iri(MODULE_IRI), module_source(MODULE_SOURCE);
	...
	public static class Vocab {
		...
		static final String BUILDER_INFERRED = ISFT_PREFIX + "builder-inferred";
		static final String BUILDER_UNINFERRED = ISFT_PREFIX + "module-build-uninferred";

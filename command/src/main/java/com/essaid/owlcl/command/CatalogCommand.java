...
public class CatalogCommand extends AbstractCommand {
	...
	@Parameter(names = "-target", description = "The top directory to start cataloging from.", converter = CanonicalFileConverter.class, validateValueWith = DirectoryExistsValueValidator.class)
	...
	@Parameter(names = "-sources", description = "The source files/directories to consider in the catalog.", converter = CanonicalFileConverter.class, variableArity = true)
	...
	@Parameter(names = "-all", description = "Create catalog files in all directories. Otherwise, only directories with "
		...
		create {
			...
			public void execute(CatalogCommand command) {
				...
				for (File file : command.getSourceFiles()) {
					if (file.isDirectory()) {
						AutoIRIMapper mapper = new AutoIRIMapper(file,
								!command.noTargetSubs);
						for (IRI oIri : mapper.getOntologyIRIs()) {
							ontologyToDocumentMap.put(oIri,
									mapper.getDocumentIRI(oIri));
						}
					} else {
						...
						try {
							...
							throw new RuntimeException(
									"Failed to load source file: " + file, e);
						}
				...
				for (File d : FileUtils.listFilesAndDirs(topDirectory,
					...
					if (command.isAll()) {
						...
						if (FileUtils.listFiles(d,
								new SuffixFileFilter(".owl"), null).size() > 0) {
							createCatalog = true;
						}
					}
					try {
						if (createCatalog) {
							...
							for (IRI iri : ontologyToDocumentMap.keySet()) {
								Path iriPath = Paths.get(ontologyToDocumentMap
										.get(iri).toURI());

								UriEntry entry = new UriEntry(
										"User Entered Import Resolution",
										catalog,
										iri.toString(),
										new URI(null, directoryPath
												.relativize(iriPath).toString()
												.replaceAll("\\\\", "/"), null),
										null);
							...
							FileWriter fw = new FileWriter(new File(d,
									command.catalogName));
							XMLCatalogWriter writer = new XMLCatalogWriter(
									catalog, fw);
							writer.write();
							fw.close();

...
// @formatter:off
/**
 * The initialization in this class is as follows:
 * <p>
 * 
 * 1. The working directory is set to "user.dir" by default. Change if needed.
 * <p>
 * 2. The properties are loaded
 * <p>
 * 2.a if there is a isft-main.properties file, in the user's home directory, it
 * is loaded.
 * <p>
 * 2.b if there is a isft-main.properties file, in the working directory, it is
 * loaded with 2.a being the parent properties (i.e. defaults that can be
 * overridden in 2.b).
 * <p>
 * 3. Default field values are set (from the properties, or other hard coded
 * defaults).
 * <p>
 * 4. The instance is passed to JCommander for further configuration from the
 * command line.
 * <p>
 * 5. Other commands are given to JCommander 5. init() is called to setup
 * logging and create directories as needed. 6. Find the
 * 
 * preInit() to set defaults and prepare for JCommander use JCommander or
 * programatically to further configure init() to do any post configuration
 * actions before running the command.
 * 
 * 
...
public class MainCommand extends AbstractCommand {
	...
	@Parameter(
			...
			description = "Suppress default console output. By default "
					...
					+ "Later, if no explicit option is available"
					+ ", the console is not quiet.")
	...
	public void configure() {
		...
		if (configProperties == null) {
			Properties homeProperties = getProperties(
					manager.getHomeDirectory(), null);
			Properties workingProperties = getProperties(
					manager.getWorkDirectory(), homeProperties);
		...
		// job qualifier
		if (!jobQualifierSet) {
			String jobQualifier = configProperties
					.getProperty(JOB_QUALIFIER_PROPERTY);
				...
				try {
					...
					if (project.isDirectory()) {
						...
						throw new IllegalStateException(
								"Project directory does not exist. Directory: "
										+ projectPath);
					}
				} catch (IOException e) {
					throw new RuntimeException(
							"Failed to get canonical File to project directory + "
									+ projectPath, e);
				}
			} else {
		...
		// ontology files
		if (!ontologyFilesSet) {
			String ontologyFileNames = configProperties
					.getProperty(ONTOLOGY_FILES_PROPERTY);
		...
		// ontology subs
		if (!ontologySubsSet) {
			String ontologyFileSubs = configProperties
					.getProperty(ONTOLOGY_SUBS_PROPERTY);
		...
		// import files
		if (!importFilesSet) {
			String importFileNames = configProperties
					.getProperty(IMPORT_FILES_PROPERTY);
		...
		// import subs
		if (!importSubsSet) {
			String importFileSubs = configProperties
					.getProperty(IMPORT_SUBS_PROPERTY);
	...
	public Object call() throws Exception {
		...
		if (outputInsideProject()) {
			throw new IllegalStateException(
					"Output directory can't be inside or equal to the project directory. "
							+ "Project: " + getProject().getAbsolutePath()
							+ " Output: "
							+ getOutputDirectory().getAbsolutePath());
		}

		getLogger()
				.info("Continuing logging in: "
	...
	private boolean outputInsideProject() {
		...
		return (getOutputDirectory().getAbsolutePath() + "/")
				.startsWith(getProject().getAbsolutePath() + "/");
	}

	public File getJobDirectory() {

		return manager.getJobDirectory(jobName + "_" + jobQualifier);
	...
	public OWLOntologyManager getNewBaseManager() {
		...
		if (ifiles == null) {
			...
			for (Entry<File, IRI> entry : ifiles.getLocalOntologyFiles(null)
					.entrySet()) {
				getLogger().debug(
						"\tIRI: " + entry.getValue() + "  <--  "
		...
		if (ofiles == null) {
			...
			for (Entry<File, IRI> entry : ofiles.getLocalOntologyFiles(null)
					.entrySet()) {
				getLogger().debug(
						"\tIRI: " + entry.getValue() + "  <--  "
								+ entry.getKey().getAbsolutePath());
	...
	private Properties getProperties(File directory, Properties parent) {
		File mainPropertiesFile = new File(directory,

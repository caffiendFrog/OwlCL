...
public class SimpleInferredModuleBuilder extends AbstractSimpleModuleBuilder {
	...
	public void run() throws Exception {
		...
		if (module.getModuleConfiguration().getSourceReasoner()
				.getUnsatisfiableClasses().getEntities().size() > 0) {
			report.info("");
			report.info("Unsatisfied entities:");
			for (OWLEntity entity : module.getModuleConfiguration()
					.getSourceReasoner().getUnsatisfiableClasses()
					.getEntities()) {
				report.info("\t" + entity);
			}
			report.info("Unsatisfieds: "
					+ module.getModuleConfiguration().getSourceReasoner()
	...
	public void addIncludes() {
		Set<OWLEntity> entities = Util.getIncludeEntities(module
				.getModuleConfiguration().getConfigurationOntology(), module
				.getModuleConfiguration().getSourceOntology(), false);

		for (OWLEntity e : entities) {
			addAxiom(module.getDataFactory().getOWLDeclarationAxiom(e));
			addAxioms(getDefiningAxioms(e, module.getModuleConfiguration()
	...
	public void addIncludeSubs() {
		Set<OWLEntity> entities = Util.getIncludeSubsEntities(module
				.getModuleConfiguration().getConfigurationOntology(), module
				.getModuleConfiguration().getSourceOntology(), false);
	...
	private void addIncludeInstances() {
		Set<OWLEntity> entities = Util.getIncludeInstances(module
				.getModuleConfiguration().getConfigurationOntology(), module
				.getModuleConfiguration().getSourceOntology(), false);
	...
	public void removeExcludes() {
		Set<OWLEntity> entities = Util.getExcludeEntities(module
				.getModuleConfiguration().getConfigurationOntology(), module
				.getModuleConfiguration().getSourceOntology(), false);
		for (OWLEntity entity : entities) {
			removeAxiom(df.getOWLDeclarationAxiom(entity));
			removeAxioms(getDefiningAxioms(entity, module
					.getModuleConfiguration().getSourceOntology(), true));

			if (entity instanceof OWLClass) {
				OWLClass c = (OWLClass) entity;
				Set<OWLClass> subs = module.getModuleConfiguration()
						.getSourceReasoner().getSubClasses(c, true)
						.getFlattened();
				for (OWLClass sub : subs) {
					...
					if (module.getBuildersClassified().containsAxiom(subAxiom)) {
						...
						;
						for (OWLClass supr : module.getModuleConfiguration()
								.getSourceReasoner().getSuperClasses(c, true)
								.getFlattened()) {
							if (module.getBuildersClassified()
	...
	public void removeExcludeSubs() {
		Set<OWLEntity> entities = Util.getExcludeSubsEntities(module
				.getModuleConfiguration().getConfigurationOntology(), module
				.getModuleConfiguration().getSourceOntology(), false);
		// report.info("Excluding class: " + entities);
		Set<OWLEntity> entityiesClosure = new HashSet<OWLEntity>();
		for (OWLEntity entity : entities) {
			entityiesClosure.addAll(OwlclUtil.getSubs(entity, true, module
					.getModuleConfiguration().getSourceReasoner()));
		}
	...
	public void addClosureToBfo() {
		for (OWLEntity entity : module.getBuildersClassified().getSignature()) {
			Set<OWLEntity> supers = OwlclUtil.getSupers(entity, true, module
					.getModuleConfiguration().getSourceReasoner());
			for (final OWLEntity supr : supers) {
				if (!supr.getIRI().toString().contains("BFO_")) {
					Set<OWLAxiom> axioms = getDefiningAxioms(supr, module
							.getModuleConfiguration().getSourceOntology(), true);
	...
	public void addAnnotations() {
		...
		entitiesToAnnotate
				.addAll(module.getBuildersClassified().getSignature());
		...
		while (entitiesToAnnotate.size() > 0) {
			...
			while (i.hasNext()) {
				...
				Set<OWLAnnotationAssertionAxiom> axioms = OwlclUtil
						.getSubjectAnnotationAxioms(module
								.getModuleConfiguration().getSourceOntology(),
								true, entity.getIRI());
				addAxioms(axioms);
	...
	public void build(IModule module, boolean inferred) {
		try {
			...
			throw new RuntimeException(
					"Error while building inferred simple module "

...
public class SimpleInferredModuleBuilder extends AbstractSimpleModuleBuilder {
    ...
    public void run() throws Exception {
        ...
        report.info("");
        report.info("==========================================================");
        report.info("======== Generating inferred simple module " + module.getModuleConfiguration().getName()
                + "================");
        report.info("===========================================================");
        report.info("");

        if (module.getModuleConfiguration().getSourceReasoner().getUnsatisfiableClasses().getEntities().size() > 0) {
            report.info("");
            report.info("Unsatisfied entities:");
            for (OWLEntity entity : module.getModuleConfiguration().getSourceReasoner().getUnsatisfiableClasses()
                    .getEntities()) {
                report.info("\t" + entity);
            }
            report.info("Unsatisfieds: "
                    + module.getModuleConfiguration().getSourceReasoner().getUnsatisfiableClasses().getEntities());
        }
    ...
    public void addIncludes() {
        Set<OWLEntity> entities = Util.getIncludeEntities(module.getModuleConfiguration().getConfigurationOntology(),
    ...
    private void addIncludeInstances() {
        Set<OWLEntity> entities = Util.getIncludeInstances(module.getModuleConfiguration().getConfigurationOntology(),
                module.getModuleConfiguration().getSourceOntology(), false);
        for (OWLEntity e : entities) {
            if (e instanceof OWLClass) {
                OWLClass classInstances = (OWLClass) e;
                Set<OWLIndividual> instances = classInstances.getIndividuals(module.getModuleConfiguration()
                        .getSourceOntology().getImportsClosure());
                for (OWLIndividual i : instances) {
                    if (i instanceof OWLNamedIndividual) {
                        OWLNamedIndividual ni = (OWLNamedIndividual) i;
                        addAxioms(getDefiningAxioms(ni, module.getModuleConfiguration().getSourceOntology(), true));
                    }
                }
            }
        }
    ...
    public void removeExcludes() {
        Set<OWLEntity> entities = Util.getExcludeEntities(module.getModuleConfiguration().getConfigurationOntology(),
                module.getModuleConfiguration().getSourceOntology(), false);
        for (OWLEntity entity : entities) {
            removeAxiom(df.getOWLDeclarationAxiom(entity));
            removeAxioms(getDefiningAxioms(entity, module.getModuleConfiguration().getSourceOntology(), true));

            if (entity instanceof OWLClass) {
                OWLClass c = (OWLClass) entity;
                Set<OWLClass> subs = module.getModuleConfiguration().getSourceReasoner().getSubClasses(c, true)
                        .getFlattened();
    ...
    public void removeExcludeSubs() {
        ...
        for (OWLEntity entity : entities) {
            entityiesClosure.addAll(OwlclUtil
                    .getSubs(entity, true, module.getModuleConfiguration().getSourceReasoner()));
        }

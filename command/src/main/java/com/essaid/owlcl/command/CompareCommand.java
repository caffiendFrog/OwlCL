...
public class CompareCommand extends AbstractCommand {
  ...
  public Object call() throws Exception {
    ...
    } else
    {
      ...
      {
        OWLImportsDeclaration id = fromManager.getOWLDataFactory().getOWLImportsDeclaration(
            entry.getValue());
        OWLOntology ontology = OwlclUtil.getOrLoadOntology(entry.getValue(), fromManager);
        if (ontology == null)
        {
    ...
    } else
    {
      ...
      for (Entry<File, IRI> entry : toOntologyFiles.getLocalOntologyFiles(null).entrySet())
      {
        OWLImportsDeclaration id = toManager.getOWLDataFactory().getOWLImportsDeclaration(
            entry.getValue());
        OWLOntology ontology = OwlclUtil.getOrLoadOntology(entry.getValue(), toManager);
        if (ontology == null)
        {
          throw new IllegalStateException("Could not load toOntology with IRI: " + entry.getValue());
        }
  ...
  enum Action {
        ...
        while (ic.next())
        {
          ...
          if (ic.getCurrentFromOntology() != null)
          {
            ...
            if (ic.getCurrentToOntology() != null)
            {
              ...
              {
                String indentString = "= " + new String(new char[ic.getDepth()]).replace('\0', '_')
                    + currentDepth + " ";
                ...
                if (ic.isCyclic())
                {
                  indentString = "  " + new String(new char[ic.getDepth()]).replace('\0', '_')
                      + (currentDepth + 1) + " ";
            ...
            } else
            {
              if (currentDepth >= depth)
              {
                String indentString = "- " + new String(new char[ic.getDepth()]).replace('\0', '_')
                    + currentDepth + " ";
                String info = indentString
                    + ic.getCurrentFromOntology().getOntologyID().getOntologyIRI();
    ...
    entities {
      ...
      public void execute(CompareCommand command) {
        ...
        for (OWLOntology bothfromOntology : command.bothFromOntologies)
        {
          ...
          Set<OWLEntity> entities = new TreeSet<OWLEntity>(bothfromOntology.getSignature(false));
          for (OWLEntity e : entities)
          {
    ...
    entitySummary {
      ...
      public void execute(CompareCommand command) {
        ...
        Set<OWLEntity> toEntities = new TreeSet<OWLEntity>(command.toOntology.getSignature(true));
    ...
    axiomSummary {
      ...
      public void execute(CompareCommand command) {
        ...
        Set<OWLAxiom> fromAxioms = new TreeSet<OWLAxiom>(OwlclUtil.getAxioms(command.fromOntology,
            true));
  ...
  public static class ImportCompare {
    ...
    ImportCompare(OWLOntology from, OWLOntology to) {
      ...
      for (OWLOntology o : to.getImportsClosure())
      {
        if (o.isAnonymous())
        {
          throw new IllegalStateException("The to ontology closure contains an anonymous ontology");
        }
      }
    ...
    boolean next() {
      if (!fromPaths.get(fromPaths.size() - 1).isEmpty() && !isCyclic())
      {
        ...
        List<OWLOntology> imports = new ArrayList<OWLOntology>(fromOntology.getDirectImports());
      ...
      } else if (!toPaths.get(toPaths.size() - 1).isEmpty() && !isCyclic())
      {
        ...
        List<OWLOntology> imports = new ArrayList<OWLOntology>(toOntology.getDirectImports());
        Collections.sort(imports);
        toPaths.add(imports);
        fromPath.add(null);
      ...
      {
        throw new IllegalStateException(
            "The ImportCompare doens't have consistent depths. Depths: " + fromPath.size() + " "
                + fromPaths.size() + " " + toPath.size() + " " + toPaths.size());
      }
      return !finished();
    }

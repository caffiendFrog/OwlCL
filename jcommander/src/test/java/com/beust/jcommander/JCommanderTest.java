...
public class JCommanderTest {
  ...
  public void simpleArgs() throws ParseException {
    ...
    String[] argv = { "-debug", "-log", "2", "-float", "1.2", "-double", "1.3", "-bigdecimal",
        "1.4", "-date", "2011-10-26", "-groups", "unit", "a", "b", "c" };
  ...
  public void repeatedArgs() {
    ...
    Assert.assertEquals(jc.getParameterDescriptions().size(), 8);
  }
  ...
  private void i18n1(String bundleName, Locale locale, String expectedString) {
    ResourceBundle bundle = locale != null ? ResourceBundle.getBundle(bundleName, locale) : null;
    ...
    ParameterDescription pd = jc.getParameterDescriptions().get(0);
  ...
  public void noParseConstructor() {
    ...
    DefaultUsage.usage(jCommander, new StringBuilder());
    // Before fix, this parse would throw an exception, because it calls
    // createDescription, which
    // was already called by usage(), and can only be called once.
    jCommander.parse();
  }

  /**
   * Test a use case where there are required parameters, but you still want to
   * interrogate the options which are specified.
   */
  public void usageWithRequiredArgsAndResourceBundle() {
    ...
    DefaultUsage.usage(jc, new StringBuilder());
    try
    {
      jc.parse("-h");
  ...
  public void converterArgs() {
    ...
    String fileName = "a";
    new JCommander(args, "-file", "/tmp/" + fileName, "-listStrings", "Tuesday,Thursday",
        "-listInts", "-1,8", "-listBigDecimals", "-11.52,100.12");
  ...
  public void commandsShouldBeShownInOrderOfInsertion() {
    verifyCommandOrdering(new String[] { "add", "commit" }, new Object[] { new CommandAdd(),
        new CommandCommit() });
    verifyCommandOrdering(new String[] { "commit", "add" }, new Object[] { new CommandCommit(),
        new CommandAdd() });
  }

  @DataProvider
  public static Object[][] f() {
    return new Integer[][] { new Integer[] { 3, 5, 1 }, new Integer[] { 3, 8, 1 },
        new Integer[] { 3, 12, 2 }, new Integer[] { 8, 12, 2 }, new Integer[] { 9, 10, 1 }, };
  }
  ...
  public void getParametersShouldNotNpe() {
    ...
    List<ParameterDescription> parameters = jc.getParameterDescriptions();
  }
  ...
  public void enumArgs() {
    ...
    Assert.assertEquals(jc.getParameterDescriptions().get(0).getDescription(), "Options: "
        + EnumSet.allOf((Class<? extends Enum>) ArgsEnum.ChoiceType.class));
  ...
  public void testListAndSplitters() {
    ...
    j.parse("-groups", "a,b", "-ints", "41,42", "-hp", "localhost:1000;example.com:1001", "-hp2",
        "localhost:1000,example.com:1001", "-uppercase", "ab,cd");
  ...
  static class V2 implements IParameterValidator2 {
    ...
    public void validate(String name, String value, ParameterDescription pd)
      ...
      names.addAll(Arrays.asList(pd.names()));
    }
  }
  ...
  public void a() {
    class Arguments {
      ...
      @Parameter(names = { "-target" }, arity = 1, description = "Target directory",
          required = true)
      public File target;

      @Parameter(names = { "-input" }, variableArity = true, description = "Input paths",
          required = true)
      public List<String> paths;
    }
    Arguments a = new Arguments();
    new JCommander(a, new String[] { "-input", "example_in1", "example_in2", "-target",
        "example_out" });
